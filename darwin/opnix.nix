# nix-darwin opnix Module - 1Password Secrets Management for macOS
#
# This module integrates opnix for declarative 1Password secrets management
# on macOS systems managed by nix-darwin.
#
...
#   - Secrets decrypted to /run/secrets/* (tmpfs equivalent on macOS)
#   - Service account should have minimal vault access (read-only)
#
# Note: On macOS, Tailscale is typically installed via Homebrew/App Store
# and authenticated interactively. This module is provided for parity and
# future use cases where automated secret injection is needed.
...
in
{
  ...
  config = lib.mkIf cfg.enable {
    ...
    # Secrets are defined by other modules that add entries to
